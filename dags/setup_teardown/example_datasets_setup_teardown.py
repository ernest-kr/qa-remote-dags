...
Trigger dataset_produces_1 runs, dataset_consumes_1 should be triggered immediately
...
# [START dataset_def]
dag1_dataset = Asset("s3://dag1/output_1.txt", extra={"hi": "bye"})
# [END dataset_def]
dag2_dataset = Asset("s3://dag2/output_1.txt", extra={"hi": "bye"})
...
with DAG(
    ...
    schedule=None,
    tags=["produces", "dataset-scheduled", "setup_teardown"],
) as dag1:
    # [START task_outlet]
    s = BashOperator(outlets=[dag1_dataset], task_id="producing_task_1", bash_command="sleep 5").as_setup()
    w = BashOperator(task_id="producing_task_2", bash_command="sleep 5")
    t = BashOperator(task_id="producing_task_3", bash_command="sleep 5").as_teardown(setups=s)
    # [END task_outlet]
    s >> w >> t
...
with DAG(
    ...
    # [END dag_dep]
    s = BashOperator(
        outlets=[Asset("s3://consuming_1_task/dataset_other.txt")],
        task_id="consuming_1",
...
with DAG(
    ...
    BashOperator(
        outlets=[Asset("s3://consuming_2_task/dataset_other_unknown.txt")],
        task_id="consuming_2",
        bash_command="sleep 5",
    )
...
with DAG(
    ...
    schedule=[
        dag1_dataset,
        Asset("s3://this-dataset-doesnt-get-triggered"),
    ],
    tags=["consumes", "dataset-scheduled"],
) as dag5:
    BashOperator(
...
with DAG(
    ...
    start_date=pendulum.datetime(2021, 1, 1, tz="UTC"),
    schedule=[
        Asset("s3://unrelated/dataset3.txt"),
        Asset("s3://unrelated/dataset_other_unknown.txt"),
    ],
    tags=["dataset-scheduled"],
) as dag6:
    BashOperator(
        task_id="unrelated_task",
        outlets=[Asset("s3://unrelated_task/dataset_other_unknown.txt")],
        bash_command="sleep 5",
    )
