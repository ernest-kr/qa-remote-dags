...
with DAG(
    dag_id="multiple_setup_teardown_per_scope",
    start_date=pendulum.datetime(2021, 1, 1, tz="UTC"),
    catchup=False,
    tags=["setup_teardown"],
) as dag:
    root_setup = BashOperator(task_id="root_setup", bash_command="echo 'Hello from root_setup'").as_setup()
    root_setup1 = BashOperator(task_id="root_setup1", bash_command="echo 'Hello from root_setup'").as_setup()
    ...
    root_teardown = BashOperator(task_id="root_teardown", bash_command="echo 'Goodbye from root_teardown'").as_teardown(
        setups=[root_setup, root_setup1]
    )
    root_teardown1 = BashOperator(
        task_id="root_teardown1", bash_command="echo 'Goodbye from root_teardown'"
    ).as_teardown(setups=[root_setup, root_setup1])

    root_setup >> root_setup1 >> root_normal >> root_teardown >> root_teardown1
