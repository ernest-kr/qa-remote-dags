...
def increment_source():
    ...
    pause.until(datetime(now.year, now.month, now.day, now.hour, pause_until_minutes, 0))
...
def source_factory(i):
    @dag(dag_id=f"source_{i}", start_date=datetime(1970, 1, 1), schedule=timedelta(days=365 * 30), tags=["datasets"])
    def source():
        increment_source()
